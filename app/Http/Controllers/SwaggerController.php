<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;

/**
 * @OA\Info(title="Wazzup News Aggregation API", version="1.0")
 * @OA\Server(url="https://localhost:7700/api")
 * @OA\Tag(name="Authentication", description="Endpoints related to user authentication")
 * @OA\Tag(name="Article Management", description="Endpoints related to article management")
 * @OA\Tag(name="Preferences", description="Endpoints related to user preferences")
 * @OA\Tag(name="Data Aggregation", description="Endpoints related to data aggregation")
 */

class SwaggerController extends Controller
{
    //
}
